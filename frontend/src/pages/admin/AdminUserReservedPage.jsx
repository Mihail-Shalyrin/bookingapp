...
export default function AdminUserReservedPage() {
  ...
  async function load(activeFilters = {}) {
    ...
    try {
      ...
      const { data } = await api.get(
        `/api/admin/users/${userId}/reserved`,
        { params }
      );
      setOffices(data);
    } catch (err) {
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ marginBottom: 16, display: "flex", gap: 8 }}>
        ...
        <button onClick={() => navigate(`/admin/users/${userId}/available`)}>
          Добавить место
        </button>
      </div>

      <h2>Резервы сотрудника</h2>
      ...
      {!loading && offices.length === 0 && (
        <p style={{ color: "gray" }}>За сотрудником нет закреплённых мест.</p>
      )}
