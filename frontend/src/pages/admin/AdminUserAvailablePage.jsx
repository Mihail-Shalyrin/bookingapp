...
export default function AdminUserAvailablePage() {
  ...
  async function load(activeFilters = {}) {
    ...
    try {
      ...
      const { data } = await api.get(
        `/api/admin/users/${userId}/available`,
        { params }
      );
      setOffices(data);
    } catch (err) {
  ...
  async function reserve(objectId, objectName) {
    if (!confirm(`Зарезервировать "${objectName}" за сотрудником?`)) return;

    try {
      await api.post("/api/admin/reserve", {
        objectId: objectId,
  ...
  return (
    <div style={{ padding: 24 }}>
      <button
        onClick={() => navigate(`/admin/users/${userId}/reserved`)}
        style={{ marginBottom: 16 }}
      >
        ← К резервам сотрудника
      </button>

      <h2>Добавить место сотруднику</h2>
      <p style={{ color: "#666", marginBottom: 16 }}>
        Свободные места, которые можно закрепить за этим сотрудником.
      </p>
